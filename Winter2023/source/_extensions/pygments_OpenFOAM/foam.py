...
"""
    ...
    Lexers for OpenFOAM languages based on Cpp lexer of pygments.
...
class OpenFOAMLexer(CFamilyLexer):
    """
    For OpenFOAM script code with preprocessor directives.
    """
    name = 'OpenFOAM'
    aliases = ['OpenFOAM', 'foam']
    filenames = ['*Dict']
    ...
    tokens = {
        'statements': [
            (words((
                ...
                'thermoType','mixture','specie','porousMedia','vertices',
                'latestTime','runTime','general','off',
                'blocks','edges','boundary','faces','simpleCoeffs','hierarchicalCoeffs','manualCoeffs','ddtSchemes',
                'gradSchemes','divSchemes','laplacianSchemes','interpolationSchemes','snGradSchemes','fluxRequired',
                'solvers','p','T','PIMPLE','SIMPLE','PISO','fields','sets','lineX1','defaultFieldValues','regions',
                'zoneToCell','fieldValues','inletValue','phi','mixture','transport','thermo','equationOfState',
                'specie','energy','molWeight','convertToMeters','hex','simpleGrading','arc','spline','q','shape','x0',
                'qmax','qmin','c','numberOfSubdomains','n',
                'delta','order','dataFile','distributed','roots','solver','preconditioner','relTol',
                'momentumPredictor','nOuterCorrectors','nCorrectors','nNonOrthogonalCorrectors',
                'ascii'), suffix=r'\b'), Keyword),
            (r'(version|format|class|object|location|application|startFrom|startTime|stopAt|endTime|adjustTimeStep|maxCo|maxDeltaT|writeInterval|writeControl|purgeWrite|writeFormat|writePrecision|writeCompression|timeFormat|timePrecision|runTimeModifiable|libs|deltaT|type|value|dimensions|internalField)\b',Keyword.Type),
