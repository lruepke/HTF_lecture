...
# V5: BLOCKED VECTORIZED PROCESSING - optimal for large meshes (1M+ elements)
# ============================================================================
# PERFORMANCE CONFIGURATION
# ============================================================================
USE_NUMBA = True
BLOCK_SIZE = 20000  # Process elements in blocks for better cache utilization
...
def make_mesh(geom: GeometryParams) -> Mesh:
    ...
    def _make_box(x: float, y: float, w: float, h: float, attribute: float,
        ...
        regions.append([x+0.01*w, y+0.01*h, attribute, 0.00001])
    ...
    def _make_inclusion(center_x: float, center_y: float, radius: float,
        ...
        regions.append([center_x, center_y, attribute, 0.00001])
...
# ============================================================================
# BLOCKED VECTORIZED ASSEMBLY FUNCTIONS
# ============================================================================
...
def assemble_system_blocked(
    ...
    """
    BLOCKED VECTORIZED element assembly - processes elements in chunks.
    
    KEY OPTIMIZATION: Processes elements in blocks for optimal cache usage.
    Each block fits in CPU cache (L3), minimizing memory bandwidth bottleneck.
    
    Perfect for LARGE meshes (1M+ elements) while maintaining vectorization benefits.
    ...
    dt : float
        Time step
    block_size : int
        Number of elements per block (default: 20000 for optimal cache usage)
    ...
    for block_idx in range(n_blocks):
        ...
        # Extract block data
        EL2NOD_block = EL2NOD[start_el:end_el]
        Kel_block = Kel[start_el:end_el]
        
        # Extract coordinates and temperatures for this block
        ECOORD_block = GCOORD[EL2NOD_block]  # shape (n_el_block, nnodel, 2)
        T_block = T[EL2NOD_block]             # shape (n_el_block, nnodel)
        
        # Storage for element matrices in this block
        Ael_block = np.zeros((n_el_block, nnodel, nnodel))
        Rhs_el_block = np.zeros((n_el_block, nnodel))
        ...
        for ip in range(nip):
            ...
            # Compute Jacobians for ALL elements in this block
            # For each element: Jac = dNds @ ECOORD^T
            Jac_block = np.einsum('ik,nkj->nij', dNds, ECOORD_block)
            
            # Vectorized inversion of all 2x2 Jacobians in block
            invJ_block, detJ_block = batch_invert_2x2(Jac_block)
            
            # Compute global derivatives for ALL elements in block
            dNdx_block = np.einsum('nij,jk->nik', invJ_block, dNds)
            
            # Mass matrix (same for all elements)
            Me = np.outer(N, N)
            
            # Stiffness matrices for ALL elements in block
            Ke_block = np.einsum('nki,nkj->nij', dNdx_block, dNdx_block)
            
            # Integration weight times determinant
            weight_detJ = weights[ip] * detJ_block
            
            # Assemble element matrices for ALL elements in block
            Ael_block += (rho * cp * Me[np.newaxis, :, :] + 
                         dt * Kel_block[:, np.newaxis, np.newaxis] * Ke_block) * weight_detJ[:, np.newaxis, np.newaxis]
            
            # Assemble RHS for ALL elements in block
            Rhs_el_block += rho * cp * (Me @ T_block.T).T * weight_detJ[:, np.newaxis]
        
        # Store coefficients for sparse matrix (vectorized indexing)
        for i in range(nnodel):
            for j in range(nnodel):
                idx = i * nnodel + j
                I[start_el:end_el, idx] = EL2NOD_block[:, i]
                J[start_el:end_el, idx] = EL2NOD_block[:, j]
                K[start_el:end_el, idx] = Ael_block[:, i, j]
        
        # Assemble global RHS by scattering element contributions
        for i in range(nnodel):
            np.add.at(Rhs_all, EL2NOD_block[:, i], Rhs_el_block[:, i])
    
    return I, J, K, Rhs_all


def compute_heat_flux_blocked(
    GCOORD: np.ndarray,
    EL2NOD: np.ndarray,
    Kel: np.ndarray,
    T: np.ndarray,
    ...
    """
    BLOCKED VECTORIZED heat flux computation - processes elements in chunks.
    ...
    for block_idx in range(n_blocks):
        ...
        # Extract coordinates and temperatures for this block
        ECOORD_block = GCOORD[EL2NOD_block]
        T_block = T[EL2NOD_block]
        
        # Compute Jacobians for ALL elements in block
        Jac_block = np.einsum('ik,nkj->nij', dNds_centroid, ECOORD_block)
        
        # Vectorized inversion
        invJ_block, _ = batch_invert_2x2(Jac_block)
        
        # Global derivatives for ALL elements in block
        dNdx_block = np.einsum('nij,jk->nik', invJ_block, dNds_centroid)
        
        # Heat flux for ALL elements in block
        Q_x[start_el:end_el] = -Kel_block * np.einsum('ni,ni->n', dNdx_block[:, 0, :], T_block)
        Q_y[start_el:end_el] = -Kel_block * np.einsum('ni,ni->n', dNdx_block[:, 1, :], T_block)
    
    return Q_x, Q_y


# ============================================================================
# SET ASSEMBLY FUNCTIONS
# ============================================================================

assemble_system_optimized = assemble_system_blocked
compute_heat_flux_optimized = compute_heat_flux_blocked
mode_name = f"BLOCKED VECTORIZED (block_size={BLOCK_SIZE})"
...
def solve_2d_temperature_fem(
    ...
    # Element assembly - BLOCKED VECTORIZED!
    ...
    if write_output:
        ...
        # Compute heat flux - BLOCKED VECTORIZED!
...
def main() -> None:
    ...
    """
    print("\n" + "="*70)
    print("2D TRANSIENT HEAT DIFFUSION - FEM SOLVER V5")
    print("BLOCKED VECTORIZED PROCESSING - Optimal for large meshes")
    print(f"Block size: {BLOCK_SIZE} elements (configurable for cache tuning)")
    print("="*70)
    ...
    # Calculate number of blocks
    n_blocks = (nel + BLOCK_SIZE - 1) // BLOCK_SIZE
    print(f"  - Number of blocks: {n_blocks} (processing {BLOCK_SIZE} elements/block)")
    ...
    writer = meshio.xdmf.TimeSeriesWriter('transient_v5.xmf')
    ...
    print(f"{'='*70}")
    print(f"Output files: transient_v5.xmf, transient_v5.h5")
    print(f"{'='*70}\n")
