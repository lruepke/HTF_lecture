...
def make_mesh(x0:float, y0:float, lx:float, ly:float, n_incl:int, radius:float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _make_box(x:float, y:float, w:float, h:float, attribute:float, vertices:list, segments:list, regions:list) -> None:
    ...
    def _make_inclusion(center_x:float, center_y:float, radius:float, points_inc:int, attribute:float, vertices:list, segments:list, regions:list) -> None:
    ...
    # generate input    
    _make_box(x0, y0, lx, ly, 1, vertices, segments, regions)

    _make_inclusion(-0.8, -0.3, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(-0.5, -0.75, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(-0.6, 0.5, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(-0.1, -0.3, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0.1, 0, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0.5, -0.2, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0.6, .3, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0.7, .8, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0, .75, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(-0.5, .05, radius, 20, 100, vertices, segments, regions)
    _make_inclusion(0.5, -.75, radius, 20, 100, vertices, segments, regions)
    ...
    return GCOORD, EL2NOD, Phases


def solve_2d_temperature_fem(EL2NOD, GCOORD, Phases, Kel, T, nip, gauss, weights, writer, rho, cp, Ttop, Tbot, dt, y0 ,lx, t):
...
def main():
    ...
    # make mesh
    GCOORD, EL2NOD, Phases = make_mesh(x0=x0, y0=y0, lx=lx, ly=ly, n_incl = n_incl, radius=radius)
    ...
    for t in range(0,nt):
        T = solve_2d_temperature_fem(EL2NOD, GCOORD, Phases, Kel, T, nip, gauss, weights, writer, rho, cp, Ttop, Tbot, dt, y0 ,lx, t)
